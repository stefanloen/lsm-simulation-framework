...
# Run reservoir continuous classification
import copy
from pprint import pprint
from analysis import get_performance_metrics
from plotting import plot_record

#chunk_mult = int(20.0/0.500) ########################################################

train_filter = np.concatenate([np.arange(0, 80), np.arange(100, 180)])
validate_filter = np.concatenate([np.arange(80, 100), np.arange(180, 200)])
...
model_report = get_performance_metrics(
...
# avg_rate = np.mean([r.reservoir_rate for r in records]) #type: ignore
# print(f"Average Reservoir Rate: {avg_rate:.2f} Hz")

pprint(model_report)

for rec in records:
    plot_record(config, rec, global_record)